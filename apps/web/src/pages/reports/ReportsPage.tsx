    ...
    <div className="flex flex-col gap-6">
        ...
        <div className="flex flex-col gap-6">
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            {[
              ...
              <div key={kpi.label} className="bg-surface-container-lowest p-6 rounded-xl border border-outline-variant shadow-sm">
                <p className="text-on-surface-variant text-sm font-medium">{kpi.label}</p>
                <p className="text-on-surface text-2xl font-bold mt-2">{kpi.value}</p>
              ...
              <div className="bg-surface-container-lowest rounded-xl border border-outline-variant shadow-sm overflow-x-auto">
                <table className="w-full text-sm">
                  <thead className="bg-surface-container border-b border-outline-variant">
                    <tr>
                      {['Date', 'Salesman', 'MS Litres', 'HSD Litres', 'Total Litres', 'Revenue', 'Cash Variance'].map((h) => (
                    ...
                    {closedShifts.length === 0 ? (
                      ...
                      closedShifts.map((shift, i) => (
                        <tr key={shift.id} className={cn('border-b border-outline-variant last:border-0', i % 2 ? 'bg-surface-container/30' : '')}>
                          <td className="px-5 py-3 text-on-surface-variant whitespace-nowrap">{formatDate(shift.closedAt ?? shift.openedAt)}</td>
                          <td className="px-5 py-3 font-medium text-on-surface">{shift.salesmanName}</td>
                          <td className="px-5 py-3 text-on-surface text-right">{shift.msLitres > 0 ? shift.msLitres.toLocaleString('en-IN') : '—'}</td>
                          <td className="px-5 py-3 text-on-surface text-right">{shift.hsdLitres > 0 ? shift.hsdLitres.toLocaleString('en-IN') : '—'}</td>
                          <td className="px-5 py-3 text-on-surface text-right">{shift.totalLitresSold.toLocaleString('en-IN')} L</td>
                          <td className="px-5 py-3 text-on-surface text-right">₹{shift.totalCashCollected.toLocaleString('en-IN')}</td>
                  ...
                  <thead className="bg-surface-container border-b border-outline-variant">
                    <tr>
                      {['Date', 'Total Litres', 'Total Revenue'].map((h) => (
                        <th key={h} className="text-left px-5 py-3 text-on-surface-variant font-semibold text-xs uppercase tracking-wider">{h}</th>
                      ))}
                    </tr>
                  </thead>
                  <tbody>
                    {dailyTotals.map((row, i) => (
                      <tr key={row.date} className={cn('border-b border-outline-variant last:border-0', i % 2 ? 'bg-surface-container/30' : '')}>
                        <td className="px-5 py-3 text-on-surface-variant whitespace-nowrap">{formatDate(row.date + 'T00:00:00')}</td>
                        <td className="px-5 py-3 text-on-surface">{row.litres.toLocaleString('en-IN')} L</td>
                        <td className="px-5 py-3 text-on-surface font-medium">₹{row.revenue.toLocaleString('en-IN')}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
          ...
          <section>
            ...
            <div className="bg-surface-container-lowest rounded-xl border border-outline-variant shadow-sm overflow-x-auto">
              <table className="w-full text-sm">
                <thead className="bg-surface-container border-b border-outline-variant">
                  <tr>
                    {['Fuel Type', 'Litres Sold', 'Rate/L', 'Gross Revenue', 'VAT Rate', 'VAT Amount', 'Net Revenue'].map((h) => (
                      <th key={h} className="text-left px-5 py-3 text-on-surface-variant font-semibold text-xs uppercase tracking-wider whitespace-nowrap">{h}</th>
                    ))}
                  </tr>
                </thead>
                  ...
                  {fuelBreakdown.map((fb, i) => {
                    ...
                    return (
                      <tr key={fb.fuelType} className={cn('border-b border-outline-variant last:border-0', i % 2 ? 'bg-surface-container/30' : '')}>
                        <td className="px-5 py-3 font-medium text-on-surface">{FUEL_LABELS[fb.fuelType]}</td>
                        <td className="px-5 py-3 text-on-surface text-right">{fb.litres > 0 ? fb.litres.toLocaleString('en-IN') : '—'}</td>
                        <td className="px-5 py-3 text-on-surface text-right">{ratePerL > 0 ? `₹${ratePerL.toFixed(2)}` : '—'}</td>
                        <td className="px-5 py-3 text-on-surface text-right">{fb.revenue > 0 ? `₹${Math.round(fb.revenue).toLocaleString('en-IN')}` : '—'}</td>
                        <td className="px-5 py-3 text-on-surface-variant text-right">{(vatRate * 100).toFixed(2)}%</td>
                        <td className="px-5 py-3 text-amber-700 font-medium text-right">{fb.revenue > 0 ? `₹${Math.round(vatAmount).toLocaleString('en-IN')}` : '—'}</td>
                        <td className="px-5 py-3 text-emerald-700 font-medium text-right">{fb.revenue > 0 ? `₹${Math.round(netRevenue).toLocaleString('en-IN')}` : '—'}</td>
                      </tr>
                    )
                  })}
                </tbody>
        ...
        <div className="flex flex-col gap-6">
          ...
          <div className="bg-surface-container-lowest rounded-xl border border-outline-variant shadow-sm overflow-x-auto">
            <table className="w-full text-sm">
              <thead className="bg-surface-container border-b border-outline-variant">
                <tr>
                  {['Date', 'Tank', 'Fuel', 'Quantity (L)', 'Rate/L', 'Total', 'Supplier', 'Invoice', ''].map((h, idx) => (
                    <th key={idx} className="text-left px-4 py-3 text-on-surface-variant font-semibold text-xs uppercase tracking-wider whitespace-nowrap">{h}</th>
                  ))}
                </tr>
              </thead>
              <tbody>
                {deliveries.length === 0 ? (
                  ...
                  deliveries.map((d, i) => (
                    <tr key={d.id} className={cn('border-b border-outline-variant last:border-0', i % 2 ? 'bg-surface-container/30' : '')}>
                      <td className="px-4 py-3 text-on-surface-variant whitespace-nowrap">{formatDate(d.deliveryDate)}</td>
                      <td className="px-4 py-3 font-medium text-on-surface">{d.tankName}</td>
                      <td className="px-4 py-3 text-on-surface">{FUEL_LABELS[d.fuelType]}</td>
                      <td className="px-4 py-3 text-on-surface text-right">{d.quantityL.toLocaleString('en-IN')}</td>
                      <td className="px-4 py-3 text-on-surface text-right">{d.ratePerLitre != null ? `₹${d.ratePerLitre.toFixed(2)}` : '—'}</td>
                      <td className="px-4 py-3 text-on-surface text-right">{d.totalAmount != null ? `₹${d.totalAmount.toLocaleString('en-IN')}` : '—'}</td>
                      <td className="px-4 py-3 text-on-surface-variant">{d.supplierName ?? '—'}</td>
                      <td className="px-4 py-3 text-on-surface-variant">{d.invoiceNumber ?? '—'}</td>
      ...
      {activeTab === 'expenses' && (
        <div className="flex flex-col gap-6">
          ...
          {expenses.length > 0 && (
            <div className="bg-secondary-container px-5 py-3 rounded-xl">
              ...
              <thead className="bg-surface-container border-b border-outline-variant">
                <tr>
                  {['Date', 'Description', 'Category', 'Amount', ''].map((h, idx) => (
                    <th key={idx} className="text-left px-5 py-3 text-on-surface-variant font-semibold text-xs uppercase tracking-wider whitespace-nowrap">{h}</th>
                  ))}
                </tr>
              </thead>
              <tbody>
                {expenses.length === 0 ? (
                  ...
                  expenses.map((exp, i) => (
                    <tr key={exp.id} className={cn('border-b border-outline-variant last:border-0', i % 2 ? 'bg-surface-container/30' : '')}>
                      <td className="px-5 py-3 text-on-surface-variant whitespace-nowrap">{formatDate(exp.expenseDate)}</td>
                      <td className="px-5 py-3 text-on-surface">{exp.description}</td>
                      <td className="px-5 py-3 text-on-surface-variant capitalize">{exp.category}</td>
                      <td className="px-5 py-3 text-on-surface font-medium text-right">₹{exp.amount.toLocaleString('en-IN')}</td>
