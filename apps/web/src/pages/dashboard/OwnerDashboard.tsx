    ...
    <div className="flex flex-col gap-8">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <div className="bg-surface-container-lowest p-6 rounded-xl border border-outline-variant shadow-sm">
          ...
          <p className={cn('text-2xl font-bold mt-2', openCount > 0 ? 'text-blue-600' : 'text-on-surface')}>
            {openCount}
          </p>
      ...
      <section>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          {tanks.map((tank) => {
            ...
            return (
              <div key={tank.id} className="bg-surface-container-lowest p-5 rounded-xl border border-outline-variant shadow-sm">
                <div className="flex justify-between items-center mb-3">
                  <div>
                    <p className="text-on-surface font-semibold">{tank.name}</p>
                    <p className="text-on-surface-variant text-xs">{tank.fuelType === 'MS' ? 'Petrol (MS)' : tank.fuelType === 'HSD' ? 'Diesel (HSD)' : 'Premium (XP)'}</p>
                  </div>
                  <p className={cn('text-2xl font-bold', isLow ? 'text-red-600' : isMed ? 'text-amber-600' : 'text-emerald-600')}>
                    {pct}%
                  </p>
                </div>
